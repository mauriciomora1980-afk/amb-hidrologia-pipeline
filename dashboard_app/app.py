...
# ─────────────────────────────────────────────
    ...
    if not df_captaciones.empty:
        ...
        parametro = st.selectbox(
            ...
            ['ICA', 'ICOMI', 'ICOMO', 'ICOSUS', 'ICOpH', 'ICOTRO'],
        )
        ...
        mask = (df_captaciones['Captacion'] == captacion) & (df_captaciones['Parametro'] == parametro)
        df_filtrado = df_captaciones[mask]
...
# ─────────────────────────────────────────────
    ...
    if not df_historico.empty:
        ...
        mask = (df_historico['Sistema'] == sistema) & (df_historico['Parametro'] == parametro)
...
# ─────────────────────────────────────────────
    ...
    if not df_embalse.empty:
        variables_comunes = [
            'pH', 'Turbiedad', 'Coliformes totales', 'Conductividad',
        ...
        if not df_filtrado.empty:
            fig = px.line(
                df_filtrado, x='Fecha', y='Valor',
                color='Sitio' if sitio == 'Todos' else None,
...
# ─────────────────────────────────────────────
    ...
    if not df_planta.empty:
        df_pivot = df_planta.pivot(
            index='Fecha_Muestra', columns='Tipo_Muestra', values='Resultado_COT_mgL'
        ).dropna()

        if not df_pivot.empty and 'AGUA CRUDA' in df_pivot.columns and 'AGUA TRATADA' in df_pivot.columns:
            df_pivot['Eficiencia (%)'] = (
                (df_pivot['AGUA CRUDA'] - df_pivot['AGUA TRATADA']) / df_pivot['AGUA CRUDA']
            ) * 100
            ...
            fig = px.line(
                df_planta, x='Fecha_Muestra', y='Resultado_COT_mgL',
                color='Tipo_Muestra',
                title=f'COT - {planta_sel} (Crudo vs Tratado)',
                markers=True,
            )
            fig.add_hline(y=5.0, line_dash="dash", line_color="red",
                          annotation_text="Límite Norma 5.0 mg/L")
            st.plotly_chart(fig, use_container_width=True)

            st.subheader("📈 Eficiencia de Remoción de COT")
            df_mostrar = df_pivot[['AGUA CRUDA', 'AGUA TRATADA', 'Eficiencia (%)']].copy()
            df_mostrar['Cumple'] = (df_pivot['AGUA TRATADA'] <= 5.0).map({True: '✅ Cumple', False: '❌ No cumple'})
            st.dataframe(df_mostrar, use_container_width=True)

            fig_ef = px.bar(
                df_pivot, x=df_pivot.index, y='Eficiencia (%)',
                title=f'Eficiencia de Remoción - {planta_sel}',
                color='Eficiencia (%)',
                color_continuous_scale=['red', 'yellow', 'green'],
            )
            fig_ef.add_hline(y=30, line_dash="dash", line_color="green",
                             annotation_text="Eficiencia objetivo 30%")
            ...
            if 'AGUA CRUDA' in resumen.columns and 'AGUA TRATADA' in resumen.columns:
                resumen['Eficiencia (%)'] = (
                    (resumen['AGUA CRUDA'] - resumen['AGUA TRATADA']) / resumen['AGUA CRUDA']
                ) * 100
...
# ─────────────────────────────────────────────
    ...
    datos_hidro = pd.DataFrame({
        'Punto': [
            'Cola del embalse', 'Centro del embalse', 'Captación del embalse',
        ...
        'Comunidad': [
            'Fitoplancton', 'Fitoplancton', 'Fitoplancton', 'Perifiton', 'Perifiton',
            'Perifiton', 'Perifiton', 'Zooplancton', 'Zooplancton', 'Zooplancton',
            'Bentónicos', 'Bentónicos',
        ],
        'Taxa': [
            'Navicula sp.', 'Navicula sp.', 'Navicula sp.', 'Navicula sp.', 'Navicula sp.',
            'Navicula sp.', 'Navicula sp.', 'Brachionus sp.', 'Brachionus sp.',
            'Brachionus sp.', 'Physidae', 'Naucoridae',
        ],
        'Abundancia': [147, 0, 34, 34, 92, 180, 43, 35, 902, 559, 4, 2],
        'Unidad': [
            'Ind/L', 'Ind/L', 'Ind/L', 'Ind/cm²', 'Ind/cm²', 'Ind/cm²', 'Ind/cm²',
            'Ind/L', 'Ind/L', 'Ind/L', 'Ind/m²', 'Ind/m²',
        ],
    })
    ...
    df_hidro = datos_hidro if comunidad_sel == 'Todas' else datos_hidro[datos_hidro['Comunidad'] == comunidad_sel]

    fig_hidro = px.bar(
        df_hidro, x='Punto', y='Abundancia', color='Taxa',
...
# ─────────────────────────────────────────────
    ...
    df_2026 = df_embalse[df_embalse['Fecha'].dt.year == 2026] if not df_embalse.empty else pd.DataFrame()
...
# ─────────────────────────────────────────────
    ...
    fig = px.bar(
        datos_surata, x='Sitio', y=parametro_sel,
